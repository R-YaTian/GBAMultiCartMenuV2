...
const int META_BLOCK_IDX = 8 * 1024 / 256 - 1; // 存放在第8MB的前一个Block
const int SRAM_NOT_SAVE_BACKUP_BLOCK_IDX = META_BLOCK_IDX - 1; // 再往前挪一个Block
// 如果一开始没选save sram, 可能导致sram炸了, 因此这个时候要把sram保存到这个位置
void loadFlashSaveToBuffer(int Game_index, bool loadFromAutoSave);
...
struct LastTimeRun{
    ...
    LastTimeRun(const volatile LastTimeRun& that){
        ...
        this->Game_index = that.Game_index;
    ...
    LastTimeRun (int _MBOffset, int _GameIndex){
        this->MBOffset = _MBOffset;
        this->Game_index = _GameIndex;
...
void saveMetaToFlash(LastTimeRun newLastRun){ // 保存LastRunGame信息的结构体
...
int askMBOffset(int lastOffset){
    ...
    while(option == -1){
        option = gameMenu.getDecision(0, 14);
    }
    int offset = gameEntries[option].MB_offset;
    int index = gameEntries[option].Game_index;
...
bool autoStartGame(){
    ...
    if(last_run.isValid()){
        if(last_run.load_from_auto_save){//如果上次游戏进过菜单，sram可能会损坏，需要从autosave中load出来
            ...
            loadFlashSaveToBuffer(-2, true); // 是不是-2都不重要，读取自动存档flag已经为true
...
void saveSramToFlash(int Game_index, bool isAutoSave){
    int blockIdx = (8 * 1024/256) + Game_index;//从8MB的地方开始放，最多支持32个游戏，32*256KB=8MB，刚好用前16MB空间
    if (isAutoSave)
...
int trySaveGame(){
    LastTimeRun last_run = *(volatile LastTimeRun*)(GAME_ROM + META_BLOCK_IDX * BLOCK_SIZE);
    if(last_run.isValid()){
        std::string gameInfoStr = "欢迎使用Ausar合卡管理菜单V0.9R\n";
        gameInfoStr += "R-YaTian(天涯)优化版\n仅供学习交流使用,禁止商业用途\n";
        gameInfoStr += "上次运行的游戏:\n游戏名: ";
        gameInfoStr += (const char*)last_run.gameName;
        gameInfoStr += "\n偏移: " + std::to_string(last_run.MBOffset) + std::string("MB");
        std::string menuTitle = gameInfoStr + "\n是否保存上次游戏存档到Flash?";

        Menu menu(menuTitle.c_str());
        menu.addOption("确定");
        menu.addOption("取消");
        menu.printList();
        int option = menu.getDecision(0, 98);
        setPos(0, 126); // 光标挪到第十行
        if (option == 0) {
            saveSramToFlash(last_run.Game_index, false);
            printf_zh("SRAM 已保存\n");
        } else {
            printf_zh("跳过保存,处理中,请稍后...\n");
            saveSramToFlash(last_run.Game_index, true); // 传递的Game_index为何值都不重要，自动保存flag已经为true
            last_run.load_from_auto_save = true;
            saveMetaToFlash(last_run);
        }
        pressToContinue(true); // 最后一行显示该信息
